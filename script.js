document.addEventListener("DOMContentLoaded", () => {
  ...
  /* =====================================================
     AUTOPLAY PETALS & MUSIC SETUP
  ===================================================== */

  // Start petal animation immediately on page load
  createPetals(18);

  // Attempt music autoplay immediately on page load
  startMusic();

  // Fallback: Autoplay audio on first user click if browser blocked autoplay
  ...
  function openInvitation() {
    ...
    setTimeout(() => {
      if (!envelope || !invitationCard) return;
  ...
  /* =====================================================
     OPEN BUTTON & ENVELOPE LISTENERS
  ===================================================== */
  ...
  function startMusic() {
    if (!weddingMusic) {
      console.warn("weddingMusic was not found.");
      return;
    }
    ...
    if (playPromise !== undefined) {
      playPromise
        ...
        .catch(error => {
          console.log("Music playback was blocked by browser policy:", error);
        });
    }
  }
  ...
  const weddingDate = new Date("September 28, 2026 8:30:00").getTime();
  ...
  function updateCountdown() {
    ...
    if (distance <= 0) {
      ...
      if (countdown) {
        countdown.innerHTML = '<div style="grid-column:1/-1"><strong>Today is the day!</strong></div>';
      }
      return;
    }
  ...
  if (gallery && slides.length > 0 && progress && current && total) {
    ...
    gallery.addEventListener("pointermove", event => {
      ...
      if (Math.abs(deltaX) > 10 || Math.abs(deltaY) > 10) moved = true;
    });
  ...
  if (rsvpForm) {
    rsvpForm.addEventListener("submit", e => {
      ...
      const mailto = `mailto:montalbangeordanlou@gmail.com?subject=${encodeURIComponent(`RSVP: ${guestName}`)}&body=${encodeURIComponent(emailBody)}`;
      const sms = `sms:+639639839550?body=${encodeURIComponent(message)}`;
